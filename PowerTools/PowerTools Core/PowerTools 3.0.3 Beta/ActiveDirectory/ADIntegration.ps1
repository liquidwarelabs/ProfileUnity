...
function Get-ProUADGroups {
    ...
    try {
        ...
        if ($response -and $response.Tag) {
            return $response.Tag | ForEach-Object {
                [PSCustomObject]@{
                    Name = $_.Name
                    SamAccountName = $_.Name  # Use Name as SamAccountName since it's not in the response
                    DistinguishedName = $_.Path
                    Domain = if ($_.Path -match 'DC=([^,]+)') { $matches[1] } else { "" }
                    GroupType = if ($_.ObjectCategory -like "*Security*") { "Security" } else { "Distribution" }
                    GroupScope = "Unknown"  # Not available in this response format
                    Description = ""
                    MemberCount = 0  # Not available in this response format
                    Members = @()
                    ManagedBy = ""
...
function Get-ProUUserGroupAccess {
    <#
    .SYNOPSIS
        Gets ProfileUnity user and group access information.
    
    .DESCRIPTION
        Retrieves user and group access data from the ProfileUnity server.
        This includes information about users, groups, and their access permissions.
    ...
    .EXAMPLE
        Get-ProUUserGroupAccess
        
    .EXAMPLE
        Get-ProUUserGroupAccess -Rows 50 -Page 1
        
    .EXAMPLE
        Get-ProUUserGroupAccess -SortBy "Name" -SortOrder "desc"
    #>
    [CmdletBinding()]
    param(
        [int]$Rows = 10,
    ...
    try {
        ...
        if (-not $serverName) {
            throw "Server name not available. Please connect to ProfileUnity server first."
        }
        
        # Build URI with parameters
        $timestamp = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
        $uri = "https://{0}:8000/api/server/user?&_search=false&nd={1}&rows={2}&page={3}&sidx={4}&sord={5}&_={6}" -f 
            $serverName, $timestamp, $Rows, $Page, $SortBy, $SortOrder, $timestamp
        
        Write-Verbose "Retrieving user/group access data from: $uri"
        ...
        # Make API call
        $apiResponse = Invoke-WebRequest -Uri $uri -Headers $headers -WebSession $session -ErrorAction Stop
        
        # Parse JSON response
        $jsonData = $apiResponse.Content | ConvertFrom-Json
        
        if ($jsonData.Tag.Rows) {
            return $jsonData.Tag.Rows
        }
        else {
            Write-Warning "No user/group access data found"
            return @()
        }
    }
    catch {
        Write-Error "Failed to retrieve user/group access data: $_"
        throw
    }
}
