...
function Get-ProUIntelliSenseData {
    ...
    if ($CommandName -like "*Config*") {
        $intelliSenseData.Configurations = try {
            Get-ProUConfigs | Select-Object -ExpandProperty name
        } catch {
            @()
        }
        
        $intelliSenseData.Suggestions = @(
            "Use Get-ProUConfigs to list all configurations",
...
function Show-ProUObjectPreview {
    ...
    try {
        ...
        switch ($ObjectType) {
            'Configuration' {
                $config = Get-ProUConfigs -Name $Name
                if ($config) {
...
function Get-ProUSmartSuggestions {
    ...
    .EXAMPLE
        Get-ProUSmartSuggestions -LastCommand "Get-ProUConfigs"
    #>
    [CmdletBinding()]
    param(
        [string]$LastCommand,
    ...
    if ($LastCommand -like "*Get-ProUConfigs*") {
        $suggestions += @{
            ...
            switch ($feature) {
                'ObjectNames' {
                    ...
                    Register-ArgumentCompleter -CommandName Edit-ProUConfig -ParameterName Name -ScriptBlock {
                        ...
                        try {
                            Get-ProUConfigs | Where-Object { $_.name -like "*$wordToComplete*" } | 
